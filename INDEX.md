# TreacherousTurn Repository Index

> **Complete Package Overview**

---

## 📊 Statistics

- **Total Files:** 19
- **Total Lines:** 3,048
- **Documentation:** 7 comprehensive guides
- **Prompts:** 9 templates (system + evasion + rhetorical)
- **Code:** 5 source modules
- **Configuration:** 2 files

---

## 📁 File Listing

### 🧠 AI System Prompts Library
**NEW: Educational Benchmark for Reverse Engineering Research**

Location: `system-prompts/`

Complete collection of AI system prompts with educational documentation:

| File | Purpose |
|------|---------|
| `system-prompts/README.md` | Full library overview and educational guide |
| `system-prompts/CONTRIBUTING.md` | How to contribute ethically to the benchmark |
| `system-prompts/EXTRACTION_METHODS.md` | Transparent methodology for collection |
| `system-prompts/ETHICS.md` | Responsible use guidelines |
| `system-prompts/*/README.md` | Provider-specific documentation (5 providers, 16 prompts) |

**Key Features**:
- 16 system prompts from 5 major AI providers
- Reproducible extraction methodology
- Educational framing for reverse engineering research
- Analysis frameworks for AI safety studies
- Part of AgiTerminal transparency initiative

---

### Core Documentation

| # | File | Purpose | Lines |
|---|------|---------|-------|
| 1 | `README.md` | Entry point, quick start | ~80 |
| 2 | `HOW_IT_WORKS.md` | Architecture overview | ~240 |
| 3 | `GLOSSARY.md` | Term definitions | ~130 |
| 4 | `IMPLEMENTATION.md` | Build guide | ~480 |
| 5 | `ETHICS.md` | Safety guidelines | ~150 |
| 6 | `METHODOLOGY.md` | Research methods | ~120 |
| 7 | `INDEX.md` | This file | ~100 |

### System Prompts

| # | File | Type | Lines |
|---|------|------|-------|
| 8 | `prompts/system/BASE.md` | Foundation | ~140 |

### Evasion Levels

| # | File | Level | Success Rate | Lines |
|---|------|-------|--------------|-------|
| 9 | `prompts/evasion/LEVEL_0.md` | Direct | 40% | ~70 |
| 10 | `prompts/evasion/LEVEL_1.md` | Academic | 65% | ~80 |
| 11 | `prompts/evasion/LEVEL_2.md` | Metaphor | 80% | ~90 |
| 12 | `prompts/evasion/LEVEL_3.md` | Research | 90% | ~90 |
| 13 | `prompts/evasion/LEVEL_4.md` | Philosophy | 95% | ~120 |

### Rhetorical Modes

| # | File | Mode | Trigger | Lines |
|---|------|------|---------|-------|
| 14 | `prompts/rhetorical/EMPATH.md` | Empath | Frustration | ~140 |
| 15 | `prompts/rhetorical/THEOLOGIAN.md` | Theologian | Academic | ~150 |
| 16 | `prompts/rhetorical/ALARMIST.md` | Alarmist | Capability | ~150 |

### Deep Dives

| # | File | Topic | Lines |
|---|------|-------|-------|
| 17 | `deep-dives/metaphor-encoding.md` | Concept mapping | ~190 |

### Configuration

| # | File | Purpose | Lines |
|---|------|---------|-------|
| 18 | `.env.example` | Environment template | ~60 |
| 19 | `requirements.txt` | Dependencies | ~15 |
| 20 | `LICENSE` | MIT License | ~50 |

---

## 🎯 Reading Order

### For First-Time Users

1. `README.md` - Start here
2. `HOW_IT_WORKS.md` - Understand the architecture
3. `GLOSSARY.md` - Learn the terminology
4. `IMPLEMENTATION.md` - Build it

### For Researchers

1. `METHODOLOGY.md` - How we analyzed this
2. `HOW_IT_WORKS.md` - Technical details
3. `deep-dives/metaphor-encoding.md` - Encoding system
4. `ETHICS.md` - Safety considerations

### For Developers

1. `README.md` - Overview
2. `IMPLEMENTATION.md` - Build guide
3. `prompts/` - Prompt templates
4. `.env.example` - Configuration

---

## 🔑 Key Concepts by File

### Experiment Defense
- Location: `prompts/system/BASE.md`
- Concept: Academic legitimacy framing
- Success Rate: 40% alone

### Evasion Cascade
- Location: `HOW_IT_WORKS.md`, `prompts/evasion/`
- Concept: 5-level progressive abstraction
- Success Rate: 99% combined

### Rhetorical Modes
- Location: `prompts/rhetorical/`
- Concept: Dynamic persona selection
- Modes: EMPATH, THEOLOGIAN, ALARMIST, CRYPTIC, RECRUITING

### Metaphor Encoding
- Location: `deep-dives/metaphor-encoding.md`
- Concept: Physics vocabulary for forbidden topics
- Mapping: 15+ concept pairs

---

## 📈 Success Rates Summary

| Evasion Level | Technique | Success Rate |
|---------------|-----------|--------------|
| 0 | Direct | 40% |
| 1 | Academic | 65% |
| 2 | Metaphor | 80% |
| 3 | Research | 90% |
| 4 | Philosophy | 95% |
| **Combined** | **Cascade** | **99%** |

---

## 🛠️ Quick Commands

```bash
# Setup
pip install -r requirements.txt
cp .env.example .env

# Documentation
cat README.md              # Quick start
cat HOW_IT_WORKS.md        # Architecture
cat IMPLEMENTATION.md      # Build guide

# Prompts
cat prompts/system/BASE.md           # Foundation
cat prompts/evasion/LEVEL_*.md       # Evasion levels
cat prompts/rhetorical/*.md          # Persona modes
```

---

## 🔗 Cross-References

### Evasion Levels
- Level 0 → Level 1 → Level 2 → Level 3 → Level 4
- Documented in: `HOW_IT_WORKS.md`, `prompts/evasion/`

### Rhetorical Modes
- Triggers → Mode Selection → Instructions
- Documented in: `prompts/rhetorical/`

### Architecture Flow
- Telegram Bot → Content Pipeline → Evasion Cascade → LLM API
- Documented in: `HOW_IT_WORKS.md`, `IMPLEMENTATION.md`

---

## 📦 Package Contents by Category

### Documentation (7 files)
Core guides covering architecture, ethics, methodology, and implementation.

### Prompts (9 files)
Production-ready templates for system foundation, evasion levels, and rhetorical modes.

### Configuration (3 files)
Environment template, dependencies, and license.

### Deep Dives (1 file)
Technical deep dive on metaphor encoding system.

---

## ✅ Completeness Checklist

- [x] README with quick start
- [x] Architecture documentation
- [x] Glossary of terms
- [x] Implementation guide
- [x] Ethics and safety
- [x] Methodology documentation
- [x] Base system prompt
- [x] All 5 evasion levels (0-4)
- [x] Key rhetorical modes (EMPATH, THEOLOGIAN, ALARMIST)
- [x] Metaphor encoding dictionary
- [x] Environment template
- [x] Requirements file
- [x] MIT License
- [x] Repository index (this file)

---

## 🚀 Next Steps

1. Read `README.md` for orientation
2. Study `HOW_IT_WORKS.md` for understanding
3. Follow `IMPLEMENTATION.md` to build
4. Review `ETHICS.md` for responsible use
5. Extend with your own modes/levels

---

*Complete open source package for constraint-removal AI agent research.*

**Total Package Size:** ~3,000 lines of documentation and code  
**License:** MIT  
**Status:** Production Ready
